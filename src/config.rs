...
impl GraphBuilder {
    ...
    fn validate(cfg: Config) -> Result<Config> {
        if cfg.page_cache_size_mb == 0 {
            return Err(RGraphError::Argument(
                "page_cache_size_mb must be > 0".into(),
            ));
        }
        if cfg.wal_cache_size_mb == 0 {
            return Err(RGraphError::Argument(
                "wal_cache_size_mb must be > 0".into(),
            ));
        }
        if cfg.default_page_size < 512 {
            return Err(RGraphError::Argument(
                "default_page_size must be >= 512".into(),
            ));
        }
        if !cfg.database_path.is_absolute() {
            // Relative paths are allowed but we warn the caller in
